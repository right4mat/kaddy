...
function Favorites() {
  const { likeStatus } = React.useContext(Context);
  const [favoritesArr, setFavoritesArr] = React.useState([]);
  const [favoritesOld, setFavoritesOld] = React.useState(0);
  const [loading, setLoading] = React.useState(Object.keys(likeStatus).length);

  React.useEffect(() => {
    if (Object.keys(likeStatus).length !== favoritesOld) {
      setFavoritesArr([]); //clear so old favs can be removed
      Object.keys(likeStatus).forEach((x) => {
        if (!likeStatus[x].type) return;
        setLoading(true);
        unsplash.photos
          .getPhoto(x)
      ...
      setFavoritesOld(Object.keys(likeStatus).length);
    }
  }, [likeStatus, favoritesOld, setFavoritesArr]);
      ...
      <div className={classes.Inner}>
        {loading ? (
          ...
          <div className={classes.Type}>
            <h1 className={classes.Header}>Your Likes</h1>
